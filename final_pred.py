...
class Application:
    ...
    def video_loop(self):
        try:
            ...
            if cv2image.any():
                ...
                if hands:  # Check if any hands detected
                    hand = hands[0]  # Get first hand
                    bbox = hand["bbox"]
                    x, y, w, h = bbox
                    image = cv2image_copy[y - offset:y + h + offset, x - offset:x + w + offset]

                    # Create fresh white image for each frame
                    ...
                    if image.size > 0:  # Check if image is valid
                        ...
                        self.ccc += 1
                        if handz:  # Check if hand detected in cropped image
                            hand = handz[0]  # Get first hand
                            self.pts = hand['lmList']  # Get landmarks

                            # Draw skeleton on white image
                            os = ((400 - w) // 2) - 15
                            os1 = ((400 - h) // 2) - 15

                            for t in range(0, 4, 1):
                                cv2.line(white_img, (self.pts[t][0] + os, self.pts[t][1] + os1),
                            ...
                            for t in range(17, 20, 1):
                                cv2.line(white_img, (self.pts[t][0] + os, self.pts[t][1] + os1),
                                         (self.pts[t + 1][0] + os, self.pts[t + 1][1] + os1), (0, 255, 0), 3)
                            cv2.line(white_img, (self.pts[5][0] + os, self.pts[5][1] + os1),
                                     (self.pts[9][0] + os, self.pts[9][1] + os1), (0, 255, 0), 3)
                            cv2.line(white_img, (self.pts[9][0] + os, self.pts[9][1] + os1),
                                     (self.pts[13][0] + os, self.pts[13][1] + os1), (0, 255, 0), 3)
                            cv2.line(white_img, (self.pts[13][0] + os, self.pts[13][1] + os1),
                                     (self.pts[17][0] + os, self.pts[17][1] + os1), (0, 255, 0), 3)
                            cv2.line(white_img, (self.pts[0][0] + os, self.pts[0][1] + os1),
                                     (self.pts[5][0] + os, self.pts[5][1] + os1), (0, 255, 0), 3)
                            cv2.line(white_img, (self.pts[0][0] + os, self.pts[0][1] + os1),
                                     (self.pts[17][0] + os, self.pts[17][1] + os1), (0, 255, 0), 3)

                            for i in range(21):
                                cv2.circle(white_img, (self.pts[i][0] + os, self.pts[i][1] + os1), 2, (0, 0, 255), 1)

                            self.predict(white_img)  # Pass the properly sized white_img

                            self.current_image2 = Image.fromarray(white_img)
    ...
    def predict(self, test_image):
        try:
            # Ensure test_image is the right shape before reshaping
            if test_image.shape != (400, 400, 3):
            ...
            pl = [ch1, ch2]
            if pl in l:
                if (self.pts[6][1] > self.pts[8][1] and self.pts[10][1] < self.pts[12][1] and self.pts[14][1] < self.pts[16][1] and self.pts[18][1] <
            ...
            if len(self.str.strip()) != 0:
                ...
                self.word = word
                if len(word.strip()) != 0:
                    suggestions = list(self.spell.candidates(word))[:4]
